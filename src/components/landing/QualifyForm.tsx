...
export function QualifyForm() {
  ...
  async function sendToWhatsApp() {
    ...
    setSending(true);
    try {
      const { ok } = openWhatsAppWithFicha(answers, result);
      if (!ok) {
        setSentFallback(true);
        const text = buildWhatsAppFicha(answers, result);
        try {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex max-h-[min(92dvh,720px)] w-full max-w-lg flex-col overflow-hidden rounded-t-[var(--radius-2xl)] border border-border bg-bg-elevated shadow-[var(--shadow-soft)] sm:rounded-[var(--radius-2xl)]">
        <div className="shrink-0 border-b border-border px-5 pb-4 pt-5 sm:px-6">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <h2
                id="qualify-title"
                className="mt-1 font-display text-xl font-extrabold tracking-tight text-fg"
              >
                {stepMeta.title}
              </h2>
            ...
            <button
              type="button"
              onClick={closeForm}
              className="flex size-9 items-center justify-center rounded-full border border-border text-fg-muted transition-colors hover:bg-white/5 hover:text-fg"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 sm:px-6">
          {step === 0 && (
            <div className="space-y-4">
              <Field label="Nome completo" error={errors.name} htmlFor="q-name">
                <input
                  id="q-name"
              ...
              <Field label="Seu WhatsApp" error={errors.phone} htmlFor="q-phone">
                <input
                  id="q-phone"
                  className={inputClass}
                  value={answers.phone}
              ...
              <Field label="Cidade" error={errors.city} htmlFor="q-city">
                <input
                  id="q-city"
                  className={inputClass}
                  value={answers.city}
            ...
            <div className="space-y-5">
              ...
              >
                ...
                <p className="mt-3 font-display text-xl font-extrabold text-fg">
                  {result.status === "qualified" && "Perfil alinhado com a formação"}
                  {result.status === "review" &&
                    "Ficha pronta para análise da equipe"}
                  {result.status === "unqualified" &&
                    "Vamos avaliar com o time"}
                </p>
              ...
              {sentFallback && (
                <div className="rounded-[var(--radius-md)] border border-gold-line/40 bg-gold-line/10 px-4 py-3 text-sm text-fg">
                  Número do WhatsApp da equipe ainda não configurado. A ficha
                  foi copiada. Cole no WhatsApp quando o número estiver ativo.
                  {!WHATSAPP_NUMBER && (
